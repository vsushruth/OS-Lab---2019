...
int main()
{
    ...
    for(i = 0; i < proNum; i++)
    {
        ...
        for( j = 0; j < partNum; j++)
        {
            diff = part[j].rem - pro[i].memory_req;
            if(diff >= 0 && temp > diff && part[j].allocated != 99)
            {
                temp = diff;
                id = j;
            }
        ...
        if(id != 999)
        {
            ...
            part[id].rem -= pro[i].memory_req;
            part[id].internal_fragmentation = part[id].rem;
            part[id].allocated++;

            if(part[id].rem == 0)
            {
                part[id].allocated = 99;
            }
