...
int main()
{
    ...
    for(i = 0; i < proNum; i++)
    {
        ...
        for( j = 0; j < partNum; j++)
        {
            diff = part[j].size - pro[i].memory_req;
            if(diff >= 0 && temp > diff && part[j].allocated == false)
            {
                temp = diff;
                id = j;
            }
        ...
        {
            pro[i].vis = true;
            pro[i].block_assigned = part[id].partID;
            
            part[id].internal_fragmentation = part[id].size - pro[i].memory_req;
            part[id].allocated = true;

            total_internal_fragmentation += part[id].internal_fragmentation;
        }
    }
