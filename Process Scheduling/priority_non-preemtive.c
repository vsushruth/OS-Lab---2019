...
int main() {
    ...
    for(i = 0; i < noPro; i++)
    {
        ...
        if(temp != i)
        {
            ...
            swap_var = pro[temp].priority;
            pro[temp].priority = pro[i].priority;
            pro[i].priority = swap_var;
    ...
    while(temp)
    {
        ...
        for(i = 0; i < noPro; i++)
        {
            if(pro[i].arrival_time <= loc_time && pro[i].priority > swap_var && pro[i].fin_flag == 0)
            {
                swap_var = pro[i].priority;
                id = i;
            }
        }
